...
5. В поле "references" укажи ТОЛЬКО те материалы, которые реально были найдены и использованы в ответе. ОБЯЗАТЕЛЬНО указывай URL каждого материала в скобках после названия.
...
1. 'internal_knowledge_search': Поиск во внутренней базе знаний. Используй для поиска регламентов, внутренних документов, а также статей законов (ГК, УК, КоАП).
   Аргументы: 
   - 'queries' (List[str], опционально): Список поисковых запросов (ОБЯЗАТЕЛЬНО ровно 4 запроса), чтобы охватить разные аспекты вопроса. Используй сгенерированные запросы для полноты поиска.
   - 'limit' (int, опционально, default=5): Количество документов на каждый запрос.

ВАЖНО: В поле 'queries' ты ОБЯЗАН указать ровно 4 поисковых запроса. Не больше и не меньше!
...
{{
    ...
    "references": ["наименование материала 1 (URL)", "наименование материала 2 (URL)"]
}}
...
1. 'internal_knowledge_search': Поиск в Системе Главбух (статьи, рекомендации, примеры, нормативные документы).
   Аргументы: 
   - 'queries' (List[str]): Список поисковых запросов (ОБЯЗАТЕЛЬНО ровно 4 запроса). Используй сгенерированные запросы для полноты поиска.
   - 'limit' (int, default=5): Количество документов на каждый запрос.
...
""",
    ...
    "filter_results_prompt": {
        "content": """Проанализируй текст материала, найденный по поисковому запросу: "{query}".
Твоя задача — извлечь фрагменты, которые в наибольшей степени отвечают на поисковый запрос.
Выбирай фрагмент не меньше одного абзаца.
Если текст содержит релевантную информацию, верни ТОЛЬКО эти фрагменты.


Текст:
{text}

Выведи только релевантные фрагменты текста или "НЕ СОДЕРЖИТ ОТВЕТА". Никакие комментарии не добавляй!.""",
        "config": {
